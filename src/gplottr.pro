...
pro gplottr,tarray,result,goodenough=goodenough,tv=tv,noconnect=noconnect, $
  x=x,y=y,_extra=eee
...
if (not keyword_set(x)) then x=640
...
if (not keyword_set(uber)) then begin
	...
	if (keyword_set(tv)) then begin
		...
		if (keyword_set(connect)) then begin
			for i=0,s(2)-1 do begin
				...
				for j=1,n_elements(w1)-1 do begin
					...
					picture(lineline(0,*),lineline(1,*)) = 0b
					pt0=pt1
				endfor
			endfor
		endif else begin
			for i=0,s(3)-1 do begin
				...
				if (ww(0) ge 0) then begin
					...
					ptrs=ww(w)
					picture(tarray(0,ptrs,i),tarray(1,ptrs,i)) = 0b
				endif
			endfor
		endelse
		tv,picture
		...
		if (ww(0) ge 0) then begin
			...
			ptrs=ww(w)
			cgplot,tarray(0,ptrs,0),tarray(1,ptrs,0),/nodata, $
				_extra=eee,/ynozero,/ysty,/xsty,/isotropic,charsize=1
			; loop over each frame
			if (keyword_set(connect)) then begin
				for i=0,s(3)-1 do begin
					w = where(tarray(0,ww,i) gt 0)
					ptrs=ww(w)
					cgplot,tarray(0,ptrs,i),tarray(1,ptrs,i),psym=3,/over
				endfor
			endif else begin
				for i=0,s(3)-1 do begin
					w = where(tarray(0,ww,i) gt 0)
	...
	if (keyword_set(tv)) then begin
		...
		if (keyword_set(connect)) then begin
			...
			for i=0L,nw-1L do begin
				j=length(1,w(i))
				traj=tarray(*,u(j)+1:u(j+1))
				picture(traj(0,*),traj(1,*)) = 0b
			endfor
		endelse
		tv,picture
		result=picture
	endif else begin
		cgplot,tarray(0,*),tarray(1,*),/nodata,/ynozero,_extra=eee,   $
			/ysty,/xsty,/isotropic,charsize=1
		u=uniq(tarray(ndat-1,*))
		ntracks=n_elements(u)
		u=[-1,u]
		if (keyword_set(connect)) then begin
			for i=0L,nw-1L do begin
				j=length(1,w(i))
				traj=tarray(*,u(j)+1:u(j+1))
				cgplot,traj(0,*),traj(1,*),/over
			endfor
		endif else begin
			for i=0L,nw-1L do begin
				j=length(1,w(i))
				traj=tarray(*,u(j)+1:u(j+1))
				cgplot,traj(0,*),traj(1,*),psym=3,/over
			endfor
		endelse
	endelse
endelse
