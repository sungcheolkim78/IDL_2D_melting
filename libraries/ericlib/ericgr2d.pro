...
function ericgr2d,data,rmin=rmin,rmax=rmax,deltar=deltar,track=track,quiet=quiet
; assumes pretrack data unless /track is used
...
theta4=checkquadrant(pos0,x0,y1,nr,rvec,rsqr,rmax,/hiy,thetax=tx,thetay=ty2)
	...
	if (t mod 5 eq 0) then begin
		if not keyword_set(quiet) then plot,result(0,*),result(1,*)/(t-tmin+1.0)
		message,string(t)+"/"+string(tmax),/inf
	endif
endfor; time for-loop

if not keyword_set(quiet) then plot,result(0,*),result(1,*)/(t-tmin)
