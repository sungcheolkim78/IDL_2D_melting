...
;    If N_elements(ecol) EQ 0 then ecol = cgcolor('opposite')
 ...
 FOR i = istart, iend, Nskip DO BEGIN     
    ...
    cgplot, [xx[i],xx[i]], [ylo[i],yhi[i]], LINESTYLE=est,THICK=eth,  $
		NOCLIP = noclip, /ADDCMD, COLOR = ecol, /overplot
...
;                                                         Plot X-error bars 
	...
	IF (hat NE 0) THEN BEGIN
		...
		cgplot, [exx1,exx2], [data_low[1,i],data_low[1,i]], $  
		  COLOR=ecol, /ADDCMD, $
          LINESTYLE=est,THICK=eth, noclip = noclip, /overplot
		cgplot, [exx1,exx2], [data_hi[1,i],data_hi[1,i]], $
		  COLOR = ecol, /ADDCMD, $
          LINESTYLE=est,THICK=eth, noclip = noclip, /overplot
