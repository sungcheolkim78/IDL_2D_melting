...
if not keyword_set(minx) then minx = min(data1)
if not keyword_set(maxx) then maxx = max(data1)

xmin=minx & xmax=maxx
if (not keyword_set(more)) then more=0
if (not keyword_set(binsize)) then begin
    dist=histogram(data1,binsize=(xmax-xmin)/99.999,rev=ri,min=xmin,max=xmax)
endif else begin
    dist=histogram(data1,binsize=binsize,rev=ri,min=xmin,max=xmax)
endelse
...
if (not keyword_set(quiet)) then message,'WARNING:'+strcompress(length-num1-num2)+ $
