; spretrack  -- revised version of epretrack (from Eric R. Weeks)
; 
; Sungcheol Kim, 2010-11-29
; Sungcheol Kim, 2011-10-26, add video input code
; Sungcheol Kim, 2011-11-05, change background substraction
;
...
;
; Add option to read sequential jpg files, and background substraction
; function, and start_frame, stop_frame option
...
pro spretrack,stk, $
	...
	back=back,avi=avi, option=option, nobpass=nobpass
...
; set frame numbers
if not keyword_set(start_frame) then start_frame = 0
if not keyword_set(stop_frame) then stop_frame = 0

if (size(stk,/type) eq 7) then begin
	if not keyword_set(jmulti) and not keyword_set(avi) then begin
		filen = findfile(stk)
		if filen(0) eq '' then message,"No file '"+stk+"' found"
		nfiles = n_elements(filen)
	endif
	if keyword_set(jmulti) then begin
		filen = stk
		nfiles = 1
	endif
    if keyword_set(avi) then begin
        filen = stk
        nfiles = 1
    endif
	usingfiles = 1
...
for j =0,nfiles-1 do begin
	...
	if (usingfiles eq 1) then begin
		if (keyword_set(jmulti)) then begin
			print, 'reading image stack: '+ffilename(filen(j),start_frame)+' to '+ffilename(filen(j),stop_frame)
		endif else begin
			print,'reading image stack: '+filen(j)
		endelse

        if (keyword_set(avi)) then stk = i_readvideo(filen(j))
		...
		;IMPORTANT: if using crop image please uncomment this sentence.
		if (keyword_set(jmulti)) then stk=readjpgstack(filen(j),start_frame=start_frame,stop_frame=stop_frame, /option)
	...
	if keyword_set(field) then begin
	   for i = 0,ns-1 do begin
		...
		im = bpassg(oddfield,bplo,bphi,/field)
	   ...
	   for i = 0,ns-1 do begin
		...
		if keyword_set(back) then img = bytscl(abs(stk(*,*,i) - back)) else img = stk(*,*,i)
        if keyword_set(nobpass) then im = img else im = bpassg(img,bplo,bphi)
	...
	if (usingfiles eq 1) then begin
		...
		if keyword_set(jmulti) then wname = wname + strtrim(string(start_frame),2)+'_' + strtrim(string(stop_frame),2)
	endif else begin
